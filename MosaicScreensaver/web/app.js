...
let artworks = [];
let tiles = [];
const TILE_TARGET_SIZE = 180; // Target width/height in pixels
let cols = 0;
let rows = 0;
            ...
            if (data.results) {
                data.results.forEach(item => {
                    if (item.artworkUrl100) {
                        // Get high-res artwork (e.g. 600x600 instead of 100x100)
                        const highResUrl = item.artworkUrl100.replace('100x100bb', '600x600bb');
                        artworks.push(highResUrl);
                    }
                });
            }
        });
...
function initGrid() {
    ...
    cols = Math.round(width / TILE_TARGET_SIZE);
    rows = Math.round(height / TILE_TARGET_SIZE);
